...
public class GalleryService {
	...
	public GalleryVo restore(GalleryVo vo, MultipartFile file) {
		...
		String saveDir = "com.javaex.jonghyun.upload";
		 ...
		 * FileOutputStream(saveDir + "/" + saveName); BufferedOutputStream bout = new
		 * BufferedOutputStream(out);
		 * 
		 * bout.write(fileData);
		 * 
		 * if (bout != null) { bout.close(); } } catch (IOException e) {
		 * e.printStackTrace(); }
		 */
		
		s3Util.fileUpload(saveDir, file, saveName);
		
		System.out.println(vo.toString());
		int count = dao.insertFile(vo);
		System.out.println("성공 횟수: " + count);
		System.out.println(vo.toString());
		vo = dao.selectByNo(vo);
		
		String url = s3Util.getFileURL(saveDir, saveName);
		vo.setUrl(url);
